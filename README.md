...
> **项目状态：** 阶段 0～4 已完成验收；阶段 5 已完成工具注册、资源清理和外部模块加载的本地验收。扩展命令注册与分发也已通过本地验收；运行结束事件订阅与监听器清理已通过本地验收；下一节点：扩展运行上下文与简易 UI。
...
阶段 5 的静态工具注册、注册资源清理及外部 JavaScript 模块加载已完成本地验收；扩展命令注册与分发也已完成本地验收；运行结束事件订阅与监听器清理已通过本地验收；下一步是扩展运行上下文与简易 UI。
...
检查扩展命令分发与保护、静态工具注册、参数校验、重名保护、卸载与失败回滚、同一对象重注册隔离、外部加载错误与项目外路径、实际请求中的工具定义及回填，并通过本地模拟服务验证会话保存恢复、工具配对、存储异常和运行诊断；包含真实 30 秒超时，运行约半分钟。
