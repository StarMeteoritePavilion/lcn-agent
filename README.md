...
> **项目状态：** 阶段 0～4 已完成；阶段 5 已验收工具与命令注册、资源清理、外部加载、运行结束事件及命令上下文与文本通知。异步命令与取消已通过本地验收；交互提问也已通过本地验收；内存待办扩展已通过本地验收；待办状态持久化已通过本地验收；异步工具节点已验收；模型只读查看待办已通过本地验收；工具权限入口已验收；下一节点：工具执行前用户确认。
...
阶段 5 当前已完成待办持久化验收。待办保存在 `.lcn-agent/todos`，重启并恢复会话后可继续查看；异步工具节点也已验收，模型只读查看待办也已验收，工具权限入口已验收，下一步是工具执行前用户确认。
...
检查扩展命令分发与保护、静态工具注册、参数校验、重名保护、卸载与失败回滚、同一对象重注册隔离、外部加载错误与项目外路径、实际请求中的工具定义及回填，并通过本地模拟服务验证会话保存恢复、工具配对、存储异常和运行诊断；包含模型和工具各一次真实 30 秒超时，完整运行约一分钟以上。
