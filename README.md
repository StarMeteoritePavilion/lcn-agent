...
> **项目状态：** 阶段 0～4 已完成；阶段 5 已验收工具与命令注册、资源清理、外部加载、运行结束事件及命令上下文与文本通知。异步命令与取消已通过本地验收；交互提问也已通过本地验收；内存待办扩展已通过本地验收；下一节点：待办状态持久化。
...
阶段 5 当前已完成按会话隔离的内存待办扩展验收，下一步是待办状态持久化。当前待办退出或卸载后会丢失。
