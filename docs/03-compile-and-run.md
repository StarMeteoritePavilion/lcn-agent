...
- [x] 本步于 2026-09-17 完成验收。

验收证据来自学习者在本次对话中提供的实际终端输出和解释：

- `npx tsc && node dist/index.js` 正常输出两种事件；读取生成文件确认类型声明和类型标注已擦除。
- 临时添加 `const message: string = 123;` 后，`npx tsc --noEmit` 报 `TS2322`。
- 删除错误实验代码后，`npx tsc --noEmit` 退出码为 `0`。
- 使用 `const message: string = "验收通过";` 并输出变量后，编译运行正常，输出“验收通过”，退出码为 `0`。
