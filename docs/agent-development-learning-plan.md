...
- [ ] 阶段 3：把核心接入真正的 TUI。
  ...
  2026-09-24：生成期间编辑的两项缺陷已修复，20 项交互检查通过；完整体验覆盖边界见验收报告。
- [ ] 阶段 4：会话保存、恢复和运行诊断。
- [ ] 阶段 5：建立可验证的扩展机制。
...
阶段总览只有在该阶段的交付物和全部验收项都通过后才勾选；当前阶段 0 至阶段 2 已完成；阶段 3 基础实现完成，完整交互体验仍待验收；阶段 4 尚未实现。
...
当前主实现 `src/index.ts` 已补充环境、工具、事件、流式请求、Agent 循环和双入口职责注释；本次仅改善可读性，未改变运行行为。

- [ ] 实现输入区、消息区、执行状态、Markdown、错误提示和取消操作。
  已验收输入提示、消息增量、轮次、工具状态、结束状态、错误、Markdown 基础渲染和取消交互。
  Markdown 已实现标题、列表、加粗、行内代码和代码块转换；复制的文本不能证明颜色和布局正确。表格保留原文。
...
- [ ] 验证工具状态更新、取消和退出后的终端恢复。
  ...
  复查命令：`npm run build && python3 tests/terminal-cancel.py`，当前工作区已通过；终端外观、退格和终端属性恢复仍需专项验证，优先由助手自动验证。
...
2026-09-24：`tests/terminal-interaction.py` 修复后 20 项检查全部通过。
中文和长行传输、多行逐行排队、短及长编辑行缩放、工具状态、退出属性及 Shell 执行通过；
生成输出混入编辑行、完成后退格继续输入的位置错误已修复，新增长输入中间光标与插入验证。
详见 [阶段 3 验收报告](stage-03-acceptance.md)。

八项交互验收：中文；长行；多行粘贴；终端缩放；生成时继续编辑；工具状态更新；取消；退出恢复终端。超出一屏的编辑内容等覆盖边界见验收报告，不代表八项全部通过。
...
阶段 2 已完成；阶段 3 基础实现完成但体验验收未闭环。接手先阅读第 8.1 节，再按门槛推进阶段 4。
...
本节纠正此前“阶段 3 全部验收完成”的结论，不改变原阶段交付目标。
...
- 2026-09-24 新增 `tests/terminal-interaction.py`，使用 pyte 验证屏幕与输入、PTY 属性和 Shell 文件副作用；修复后 20 项全部通过。前述临时脚本局限保留作历史说明，最新覆盖以 [验收报告](stage-03-acceptance.md) 为准。
...
该测试覆盖空行提示、取消时补齐半行、取消后继续请求、空闲 Ctrl+C 和 `/exit`，不检查屏幕布局。
...
### 立即下一节点：明确并补齐阶段 3 剩余体验边界

2026-09-24 原两项交互缺陷已按用户授权修复，20 项交互检查、配置测试、取消测试和类型检查通过。
事件输出统一协调编辑行，`prompt(true)` 保留光标；长输入缩放及中间插入已验证。
完整步骤与剩余边界见 [验收报告](stage-03-acceptance.md)：超过一屏的编辑内容、滚屏历史及终端差异
尚未验证，不以这次 20 项通过代替全部交互体验。当前两项缺陷不再列为待办。
阶段 3 未闭环前，不自动开始阶段 4 实现；用户明确决定先推进阶段 4 时记录暂缓项。

### 阶段 4 首个完整节点：JSONL 线性会话保存与恢复

状态：教学方案待展开，尚未写入实现。前置：阶段 3 验收闭环，或用户明确允许保留体验缺口继续。
