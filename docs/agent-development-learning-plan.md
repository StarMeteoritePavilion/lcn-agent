...
进度更新：2026-09-25。阶段 4 已验收完成，阶段 5 尚未开始；当前入口仍保留阶段 4 实现。
...
- [x] 阶段 3：把核心接入真正的 TUI。
- [x] 阶段 4：会话保存、恢复和运行诊断。
  会话恢复、运行诊断、错误与取消、30 秒超时及中断状态均通过当前源码的本地模拟验收。
- [ ] 阶段 5：建立可验证的扩展机制。
...
阶段 4 已完成当前主实现的本地模拟验收，见本文 [验收记录](#验收记录)。
下一步进入阶段 5：先用静态函数注册验证扩展入口，再讨论外部模块加载。
按学习模式明确工具定义、运行时参数校验、执行函数与注册表的职责，使用真实调用方验证重名报错和清理。
本次不提前定义扩展 API 或写入实现，也不自动删除阶段 4 测试；进入新节点时再调整当前验收入口。
