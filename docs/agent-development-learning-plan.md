...
- [ ] 完成六项验收：纯文本结束、工具回填后继续、未知工具、参数不合法、执行异常、轮次上限。
  已验证未知工具、参数不合法和工具执行异常均转换为失败的 `ToolResult`，并继续第二次模型调用；纯文本结束和轮次上限待完成。
  已验证纯文本直接结束，循环场景在第 3 轮发出 `loop_limit` 并停止；六项验收尚待统一复核。
...
工具参数校验、未知工具、执行异常、运行事件和最大轮次限制均已完成运行验证；下一步是统一完成阶段 1 六项验收。
