...
进度更新：2026-09-26。阶段 4 已验收完成；阶段 5 的静态工具注册已通过主实现验收。注册资源归属与清理已通过主实现验收。已编译 JavaScript 外部模块加载已通过主实现本地验收；扩展命令注册与分发已通过主实现本地验收；运行结束事件订阅与监听器清理已通过主实现本地验收。命令运行上下文与简易通知 UI 已通过主实现本地验收；异步命令与取消已通过主实现本地验收；交互提问已通过主实现本地验收；按会话隔离的内存待办扩展已通过主实现本地验收；待办状态持久化已通过主实现本地验收；工具运行上下文与异步工具执行已验收；用户提供真实模型成功调用证据。模型只读查看待办已通过本地验收；工具权限判定入口已通过本地验收；下一节点为工具执行前用户确认，尚未开始。
...
阶段总览只有在该阶段的交付物和全部验收项都通过后才勾选；当前阶段 0 至阶段 4 已完成本地验收；阶段 3 当前采用 readline 与 ANSI 渲染，尚未接入 Pi TUI。
...
静态工具注册已接入主项目。`src/core/tools.ts` 管理注册表与扩展装载，
`src/extensions/echo.ts` 登记定义并校验参数，`src/index.ts` 统一装载和退出清理。

### 当前实现与关键语法

- `ToolRegistry.register(tool)` 返回 `() => void`，负责撤销这一次注册。
- `RegisterTool` 仍返回 void；扩展通过 ExtensionAPI 的 registerTool 与 registerCommand 登记能力，宿主收集实际的清理函数。
- `mountExtension(registry, setup)` 为每次装载创建独立清理集合，返回统一卸载函数。
- 初始化中途失败，逆序撤销本次已登记的工具，并原样抛出错误；其他扩展工具保留。
- 卸载后，即便扩展保存了注册函数，继续调用也会被拒绝。
- 入口在最外层 finally 清理；正常退出或初始化失败都会进入该清理路径。

`(tool: Tool) => () => void` 表示“接收工具，返回无参数、无返回值的函数”。
调用 register 时保存工具，调用返回函数时才删除工具。
`Array<() => void>` 是清理函数数组，资源拥有者负责释放，类似 Java 中的 close。

每一次注册都用闭包保存独立的 `active` 状态。第一次撤销后状态失效，后续调用直接返回。
不能只比较工具对象：同一个对象可能在撤销后重新注册，旧撤销函数必须保持失效，不能删除新注册。
扩展层也有独立的 active，用于阻止重复卸载及卸载后的继续注册。

### 验证与边界

生命周期断言已合入唯一验收脚本 `tests/session-persistence.py`，执行：
...
覆盖卸载后不可发现及不可执行、重复清理、连续三次重载、初始化失败回滚、
重名失败不影响已有工具、失效注册拒绝，以及复用同一工具对象时旧撤销函数不误删新注册。
同时保留静态注册、模型工具定义、会话与诊断的本地模拟集成验证。

边界：setup 必须是同步函数，只登记工具和命令，不启动定时器、连接或后台工作。
“回滚”只撤销宿主管理的注册，不能撤销扩展自己发出的网络或文件操作。
只在运行结束后卸载，不提供运行中热重载命令；清理集合只包含宿主的工具和命令删除函数。
异步工具、其他事件、UI 和状态接口尚未实现，阶段 5 整体仍未完成。

### 外部 JavaScript 模块加载（已完成本地验收）

入口通过进程环境变量 `LCN_AGENT_EXTENSION` 读取一个可信本地模块路径，
使用 Node 原生 `import()` 加载默认导出，再由 `mountExtension()` 完成同步注册及清理。
路径相对于启动目录解析；在 loadConfig 前读取，因此需从启动命令设置，不从 `.env` 获取。
未设置时仅启用内置 echo；设置后另外加载外部工具。

学习用 upper 编译为 `dist/extensions/upper.js`，入口不静态导入它。
2026-09-25：当前源码通过 `npm run check`、`npm run build`、`npm test`，退出码均为 0。
外部加载断言已合入唯一验收脚本，覆盖：

- 内外工具共存、参数校验、重名保护、连续三次装载与卸载。
- 缺文件、非函数默认导出、仅命名导出、模块执行异常、初始化部分注册后失败。
- 错误包含绝对路径和 cause，失败后保留原有工具且无部分注册残留。
- 将真实 upper 编译产物复制为项目外 `.mjs`，路径包含中文、空格及 `#`，仍能加载。
- 实际入口向本地模拟模型提供 echo 与 upper，upper 调用结果 HELLO 正确回填。
- 缺文件时入口退出码为 1，不请求模型；既有会话、诊断、取消和真实 30 秒超时验收仍通过。

本轮未请求真实模型。需要真实模型抽查时，可在构建后运行：

```bash
LCN_AGENT_EXTENSION=./dist/extensions/upper.js node dist/index.js "请调用 upper 工具，将 hello 转为大写"
```

Node 缓存同一 URL 的模块；重新装载只重新调用注册函数，不承诺重新读取修改后的源码。
模块加载会执行顶层代码，只加载可信模块，注册回滚不是沙箱；初始化仍须遵守同步注册约定。

### 扩展命令注册与分发（已完成本地验收）

ExtensionAPI 统一注册工具与用户命令，支持命令分发、宿主名称保护、失败回滚和卸载。
当前源码类型检查、构建及唯一验收脚本全部通过；新增命令断言验证名称与重名保护、
工具与命令共同回滚、错误传播、失效注册拒绝、同一对象重复注册清理隔离。
交互入口验证空格及 Tab 参数、未知和空命令、报错后继续、无模型请求且不创建会话。
现有工具、外部加载、会话、诊断和真实 30 秒超时检查仍通过。未请求真实模型。

### 运行结束事件订阅与监听器清理（已完成本地验收）

当前源码类型检查、构建、完整 npm test 及差异检查通过。事件断言已合入唯一脚本，
覆盖跨扩展通知顺序、冻结快照、同步异常隔离、分发期间增删、失败回滚、失效 API 拒绝、
同回调多次订阅的独立清理，以及三次外部装载无重复计数。
入口验证成功和模型错误均计数，命令不计数，切换会话保留装载级计数。
本节点仅承诺同步观察回调；不支持异步监听器，也不作为权限拦截器。
本轮未请求真实模型；取消路径及真实 30 秒超时原有检查通过，未单独验证这两种路径的 /runs 数值。

### 命令运行上下文与简易通知 UI（已完成本地验收）

当前源码类型检查、构建、完整 npm test 和差异检查通过；唯一脚本已补齐命令上下文参数。
真实入口检查上下文及 ui 均被冻结，修改会话字段或替换 notify 均被拒绝；
通知正确输出工作目录、模型及会话，void 返回值不显示 undefined。
无会话、连续新建、重启恢复、旧命令兼容及不请求模型均通过；用户提供的实际终端输出也符合预期。
修正测试对 macOS 临时目录符号链接路径的比较，使用解析后的真实路径；应用实现未修改。
当前仅支持同步命令上下文与文本通知；未请求真实模型，未新增终端屏幕布局专项检查。

### 异步命令与取消（已完成本地验收）

当前源码类型检查、构建、完整 npm test 及差异检查通过。唯一脚本已适配 Promise 返回值与 signal，
新增异步异常、执行中取消、预先取消阻止执行及同步命令兼容断言。
真实 PTY 验证 /wait 正常完成、Ctrl+C 取消、取消后继续执行命令、排队输入串行执行和正常退出。
用户提供的正常完成、取消和继续执行输出亦符合预期；本轮未请求真实模型。
2026-09-26 复查：完整验收再次通过，补齐入口异步拒绝后恢复、忽略信号时迟到返回值被丢弃、
stdin 关闭触发取消并退出；修正“等待期间不接受输入”的注释，实际为后续输入排队执行。
取消依赖命令协作，不能强制终止忽略 signal 的扩展；本节点未引入异步事件监听器。

### 交互提问（已完成本地验收）

2026-09-26：类型检查、构建、完整 npm test 和差异检查通过。
真实 PTY 验证中文、空回答、斜杠文本回答、并发提问拒绝、取消后重新提问、输入关闭；
管道输入拒绝提问且仍可执行普通命令，回答不请求模型、不创建会话。
上述检查已合入唯一脚本；应用实现未改动，未请求真实模型。
回答须在问题提示出现后输入；提前排队文本仍按普通输入处理，不支持命令与答案提前一起粘贴。

### 按会话隔离的内存待办扩展（已完成本地验收）

2026-09-26：当前源码类型检查、构建、完整 npm test 和差异检查通过。
唯一脚本已加入新增、提问、完成、重复完成、空内容、精确编号、取消不写入、会话隔离及卸载检查。
真实入口验证切换与切回会话，PTY 验证提问新增、取消不占编号；命令不请求模型。
同一实现通过静态 mountExtension 和外部 loadExtension 装载检查，主入口当前使用外部 todo 扩展。
用户提供的原会话两项状态和另一会话空列表与测试一致。
该历史节点仅使用内存，退出或卸载会丢失待办；当前实现已由下述持久化节点替代。
本轮未请求真实模型，未修改应用实现。

### 待办状态持久化（已完成本地验收）

2026-09-26：当前源码类型检查、构建、完整 npm test 及差异检查通过。
每条命令读取 `.lcn-agent/todos/<会话文件名>.json`，校验版本、UTF-8、JSON 和条目结构，
保存使用同目录临时文件替换。新增断言覆盖重装恢复、状态保存、会话隔离、非法路径、
损坏文件保留、取消不写入、替换失败及临时目录清理；真实入口重启恢复和 PTY 取消检查通过。
应用执行逻辑未修改，仅收紧两处注释中的保证范围；未请求真实模型。

边界：依赖宿主单写入进程，不保证外部并发修改、删除或断电后的目录项持久性。
本轮保存失败通过目标目录冲突验证，未模拟全部磁盘故障；替换成功后若清理抛错，数据可能已保存。
当前为待办专用存储，尚未提供通用扩展会话状态 API。

### 工具运行上下文与异步执行（已完成本地验收）

ToolContext 提供会话标识、调用 ID、工作目录、模型和 signal，工具执行返回 Promise。
取消时仍为本批调用保存配对结果，后续调用标记未执行且不再请求模型。
沿用本轮 signal，因此 30 秒预算包含模型请求及串行工具执行；本节点不引入工具提问或权限判定。
当前已手敲接入；2026-09-26 修复超时误报取消，新增 timeout 运行结束原因及独立界面提示。
类型检查、构建及完整 npm test 通过，包含模型与工具各一次真实 30 秒超时、
超时后同批结果配对、停止后续模型请求及会话恢复检查。
测试已适配异步工具接口。用户随后提供两次真实 delay_echo 调用成功输出：工具返回后，第二轮模型正常 stop。
该输出补齐真实模型正常调用证据，不证明精确等待时长；此前服务响应慢的具体原因仍未确定。
### 模型只读查看待办（已完成本地验收）

2026-09-26：经用户授权直接修改，todo_list 工具与命令共用读取和展示，更新相关注释。
工具仅允许空对象参数，不能指定其他会话；会话来自宿主 ToolContext。
类型检查、构建、完整 npm test 与差异检查均通过。验证空列表不创建文件、参数拒绝、
会话隔离、损坏保留、卸载失效，以及本地模拟模型发现工具、正确结果回填和待办文件字节不变。
未请求真实模型，新增与完成仍仅通过用户命令执行。
### 工具权限判定入口（已完成本地验收）

2026-09-26：类型检查、构建、完整 npm test 及差异检查通过。
默认拒绝、严格 true 允许、同步与异步策略异常、非布尔拒绝、判定期间取消检查通过。
实际入口验证未授权工具没有副作用、拒绝结果正确配对回填；既有模拟测试明确注入允许策略。
用户已提供真实模型拒绝结果，并确认恢复名单后的允许场景正常。当前允许名单已恢复。
本节点是可信扩展的名称准入，不是参数级审批、副作用证明或恶意代码沙箱。
下一节点：工具执行前用户确认，先用 delay_echo 验证，之后再接待办写入工具。
阶段 5 整体仍未完成。
...
| 2026-09-25 阶段 4 运行诊断 | 当前主源码类型检查及诊断验收通过，命令退出码均为 0 | `npm run check`、`npm test` |
| 2026-09-25 阶段 5 静态注册主实现 | `npm run check`、`npm run build`、`npm test` 均退出 0；注册、参数边界、重名、异常传播、模型工具定义及会话诊断集成通过 | 本节点完成，阶段 5 未完成 |
| 2026-09-25 阶段 5 注册资源清理 | 修复同一对象重新注册时旧清理误删；类型检查、构建及包含生命周期断言的完整 npm test 均退出 0 | 本节点完成，未请求真实模型，阶段 5 未完成 |
| 2026-09-25 阶段 5 外部模块加载 | 当前源码类型检查、构建和完整模拟验收通过，含项目外路径及工具回填 | 本节点完成，未请求真实模型，阶段 5 未完成 |
| 2026-09-25 阶段 5 扩展命令 | 类型检查、构建、完整 npm test 均退出 0；命令保护、回滚、清理及交互分发通过 | 本节点完成，未请求真实模型，阶段 5 未完成 |
| 2026-09-25 阶段 5 运行结束事件 | 类型检查、构建、完整 npm test 均退出 0；事件隔离、生命周期及入口计数通过 | 本节点完成，阶段 5 未完成 |
| 2026-09-25 阶段 5 命令上下文 | 类型检查、构建、完整 npm test 通过；入口冻结、通知、会话切换及恢复通过 | 本节点完成，阶段 5 未完成 |
| 2026-09-25 阶段 5 异步命令 | 类型检查、构建、完整 npm test 通过；异步异常、协作取消及真实终端排队通过 | 本节点完成，阶段 5 未完成 |
| 2026-09-26 阶段 5 交互提问 | 类型检查、构建、完整 npm test 通过；PTY 回答、取消、并发及非终端边界通过 | 本节点完成，阶段 5 未完成 |
| 2026-09-26 阶段 5 内存待办 | 类型检查、构建、完整 npm test 通过；状态操作、取消、会话隔离及内外装载通过 | 历史内存节点完成，现已接入持久化 |
| 2026-09-26 阶段 5 待办持久化 | 类型检查、构建、完整 npm test 通过；重启恢复、数据保护与替换失败通过 | 本节点完成，阶段 5 未完成 |
...
当前唯一脚本 `tests/session-persistence.py` 已增加静态工具注册及生命周期检查，并继续覆盖多轮历史、新会话隔离、重启恢复不重放工具、
取消恢复、损坏记录、模型不匹配、工具配对、独占锁和写入失败。文件丢失断言已合入：
不重建无头文件，内存消息及配对状态不变；不保证抵御外部进程在打开描述符后删除或替换文件。
诊断验收覆盖耗时、提供商用量、缺失用量为 null、工具调用关联、模型和工具错误、主动取消、
真实 30 秒超时、查看不重放、中断记录显示未知、尾行损坏保持原文件、无日志会话和诊断路径创建失败。
模型原始错误正文未写入诊断。中断场景用开始后无结束的记录验证，并非任意故障点强杀测试；
路径失败不等同于模拟所有磁盘写入故障。未进行真实模型抽查，本地验收已满足本阶段清单。
