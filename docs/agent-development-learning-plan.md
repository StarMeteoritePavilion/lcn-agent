...
进度更新：2026-09-26。阶段 0～5 已完成本地基础范围验收；阶段 5 入口已保存为 src/stage-05.ts，核心与扩展的完整版本由 Git 提交记录。下一节点为阶段 6 的默认扩展装配，尚未实现。
...
- [ ] 默认扩展装配：无需环境变量即可使用提问与待办，外部扩展仍可选加载；验证重名保护与退出清理。
...
阶段 5 收尾核对通过：外部扩展无需改核心、工具重名报错、加载错误定位、卸载无注册残留、连续三次装卸无重复处理、多个扩展按订阅顺序通知，均有现有脚本证据。提问、待办、通用状态和同步自有资源清理也已通过本地验收。沿用同步清理节点的完整回归结果；本次新增入口快照只做一致性和类型检查，不重复请求真实模型。下一节点是阶段 6 默认扩展装配，目前仍须显式加载 workflow.js。
