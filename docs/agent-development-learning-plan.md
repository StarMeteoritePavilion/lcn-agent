...
八项交互验收：中文；长行；多行粘贴；终端缩放；生成时继续编辑；工具状态更新；取消；退出恢复终端。现有 PTY 证据只覆盖部分行为，不代表八项全部通过。
...
- `src/index.ts` 是主实现；`src/stage-00.ts`、`src/stage-01.ts`、`src/stage-02.ts` 仅留档。
...
真实模型单次入口使用 `npm start -- "你好"`；直接运行 dist 时需 `node --env-file=.env dist/index.js "你好"`。
不读取或记录 `.env` 中的密钥。当前 `npm test` 是占位命令，不作为验收命令。

### 立即下一节点：阶段 3 交互证据补齐

先补一个仓库内可复跑的整体验证：精确核对服务端收到的中文、长行和排队输入；
验证生成中编辑、退格及缩放后的屏幕内容；比较退出前后的终端属性并确认 Shell 命令实际执行，
不能仅匹配命令回显。需要终端仿真才能验证的屏幕效果单独记录，不以进程存活代替。
如暴露实现缺口，报告具体触发方式和影响，按学习模式给出一个完整修复节点。
