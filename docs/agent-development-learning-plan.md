...
进度更新：2026-09-25。阶段 4 已验收完成；阶段 5 的静态工具注册已通过主实现验收。注册资源归属与清理已通过主实现验收。已编译 JavaScript 外部模块加载已通过主实现本地验收；下一节点为扩展命令注册与分发，尚未开始。
...
- `RegisterTool` 仍返回 void；扩展只负责登记，宿主收集实际的清理函数。
...
边界：setup 必须是同步函数，只登记工具，不启动定时器、连接或后台工作。
“回滚”只撤销宿主管理的注册，不能撤销扩展自己发出的网络或文件操作。
只在运行结束后卸载，不提供运行中热重载命令；清理集合只包含宿主的工具删除函数。
异步工具、扩展命令、事件、UI 和状态接口尚未实现，阶段 5 整体仍未完成。
...
将扩展可注册的能力从工具拓展到用户命令，复用重名检查、归属、失败回滚和卸载规则。
先用一个简单命令验证：终端输入直接分发给扩展，不进入模型请求；卸载后不可调用。
区分用户主动输入的命令与模型发起的工具调用，并保护已有会话命令不被扩展覆盖。
按文件位置和必要片段提供教学，主实现尚未开始。
事件、简易 UI、会话状态、用户提问与待办扩展、权限拦截和完整重载验收仍待完成，阶段 5 不勾选完成。
