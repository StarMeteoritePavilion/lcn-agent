...
已完成范围以第 4 节清单为准。经确认的待办新增与完成工具已通过主源码类型检查、构建和完整验收。命令与工具共用存储，真实 PTY 验证审批结果回填、拒绝与取消文件不变、非交互拒绝；未请求真实模型。下一节点为通用扩展会话状态接口，尚未开始。
教学代码与修改片段只在对话展示，本节原地更新当前状态，不按每次推进追加章节。

- 扩展初始化和事件监听器保持同步；命令与工具执行支持异步及协作取消。进程内扩展必须可信，注册 API 不是沙箱。
...
最近一次主源码类型检查、构建、完整 npm test 和差异检查均通过。测试使用隔离目录和本地模拟服务，未请求真实模型。
