...
阶段总览只有在该阶段的交付物和全部验收项都通过后才勾选；当前完成了阶段 0 的初始化、依赖安装、编译运行、事件联合类型和类型收窄步骤。
...
- [ ] 完成 `Promise`、`async/await` 和异常传播练习。
...
- [ ] 读懂一次用户请求、一次模型响应和一次工具调用之间的关系。
- [ ] 实现模拟模型：第一次返回工具调用，收到结果后第二次返回最终文本。
- [ ] 实现一个回显工具、消息历史和串行工具执行。
- [ ] 输出模型、工具开始、工具结束和最终回答等运行事件。
- [ ] 增加最大轮次限制，并让循环在达到限制后停止。
- [ ] 完成六项验收：纯文本结束、工具回填后继续、未知工具、参数不合法、执行异常、轮次上限。
...
- [ ] 接入一个实际可用的模型提供商，并确认配置来源。
- [ ] 实现文本增量、完整工具调用、用量展示、超时和主动取消。
- [ ] 区分提供商报告用量、估算用量和缺失用量。
- [ ] 验证流式参数未完成或响应截断时不执行工具。
- [ ] 验证停止后不自动启动下一次模型调用。
- [ ] 完成流式问答、工具循环、请求失败和主动取消四项验收。
...
- [x] 指定新项目目录，保留 Pi 仓库作阅读参考。本次只创建此计划，没有新建 Agent 工程。
- [ ] 初始化 TypeScript 严格模式与一个可运行入口，锁定实际安装依赖版本。
- [x] 用联合类型表达消息与运行事件，先不加入 TUI 和真实 SDK。
- [ ] 实现模拟模型：先请求回显工具，收到结果后返回最终文本。
- [ ] 实现参数校验、执行、回填、最大轮次；完成阶段 1 的六项验收。
- [ ] 用一段事件日志解释为什么发生了两次模型调用、工具执行发生在哪一步、为何最后停止。

这一单元完成后再接真实模型。业务接入、插件市场、远程 Agent 服务和分布式任务调度不作为开工前置条件。
我们承诺最终默认扩展范围，但按阶段实现，不在第一天搭建全部抽象。
