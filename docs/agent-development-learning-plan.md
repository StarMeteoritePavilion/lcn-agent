...
进度更新：2026-09-26。阶段 0～4 已完成；阶段 5 已完成至工具执行前用户确认，整体尚未结束。经确认的待办新增与完成工具已验收；下一节点是通用扩展会话状态接口。
...
- [ ] 提供扩展自行创建资源的清理登记机制，验证初始化失败、卸载及退出时的清理。
...
已完成范围以第 4 节清单为准。经确认的待办新增与完成工具已通过主源码类型检查、构建和完整验收。命令与工具共用存储，真实 PTY 验证审批结果回填、拒绝与取消文件不变、非交互拒绝；未请求真实模型。2026-09-26 复核补齐已有装卸、事件顺序与权限失败验收的勾选；通用扩展会话状态接口、提问与待办完整集成、自有资源清理登记仍未完成。下一节点为通用扩展会话状态接口，尚未实现。
教学代码与修改片段只在对话展示，本节原地更新当前状态，不按每次推进追加章节。

- 扩展初始化和事件监听器保持同步；命令与工具执行支持异步及协作取消。当前卸载只撤销注册的工具、命令和事件订阅，不管理扩展自行创建的定时器、连接或子进程。进程内扩展必须可信，注册 API 不是沙箱。
- 外部扩展通过启动环境变量 `LCN_AGENT_EXTENSION` 指定；当前只加载一个。Node 缓存模块，重新装载不等于刷新源码。
...
- 待办使用独立的版本化 JSON，按会话隔离，重启可恢复；尚未抽象为通用扩展状态 API。
- 存储依赖宿主单写入进程，不保证外部并发改写或断电后的目录项持久性。替换成功后的清理异常可能意味着数据已保存。

## 验收记录

仅维护 `tests/session-persistence.py` 一个脚本，运行方式见 [README](../README.md#当前阶段验收)。
2026-09-26 文档复核时重新运行 `npm run check`、`npm run build`、完整 `npm test` 和 `git diff --check`，均通过。测试使用隔离目录和本地模拟服务，未请求真实模型。阶段 0～4 的历史快照本次只做源码核对与类型检查，未重跑已移除的历史专项验收。
...
| 2026-09-25～26 阶段 5 扩展基础 | 静态注册、外部加载、命令、事件顺序、失败回滚、卸载及重复装载通过 |
| 2026-09-26 阶段 5 上下文与提问 | 只读上下文、会话切换、异步命令、输入排队、提问与取消、非终端拒绝通过 |
| 2026-09-26 阶段 5 待办 | 新增/完成/查看、持久化恢复、会话隔离、损坏保留及模型读写工具回填通过；写入须确认 |
