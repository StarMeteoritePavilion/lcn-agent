...
阶段总览只有在该阶段的交付物和全部验收项都通过后才勾选；当前阶段 0 已完成，阶段 1 尚未开始验收。
...
阶段 1 的第一个单元是实现不依赖真实模型的最小工具循环：

- [x] 描述一次用户请求、一次模型响应和一次工具调用的关系。
- [x] 实现模拟模型：第一次返回回显工具调用，收到工具结果后返回最终文本。
- [x] 实现回显工具、消息历史和串行工具执行。
- [x] 输出模型响应、工具执行结果和最终回答。
- [x] 用运行输出解释两次模型调用、工具执行位置和流程停止原因。
  2026-09-18：正常输入和固定工具文本实验均已运行；已确认模型只返回工具调用，Agent 执行工具并回填结果。

下一单元是把当前手动串联的两次模型调用抽成循环，再继续参数校验、未知工具、执行异常和最大轮次限制。
完成阶段 1 前不接入真实模型、TUI 或 MCP。
