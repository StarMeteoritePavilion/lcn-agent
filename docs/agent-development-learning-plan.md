...
进度更新：2026-09-26。阶段 0～4 已完成；阶段 5 已完成提问与待办的组合集成及通用会话状态接口，整体尚未结束。下一节点是扩展自有资源清理登记。
...
已完成范围以第 4 节清单为准。workflow.ts 组合 upper 与 todo，已通过类型检查、构建及完整验收：同一终端的提问与待办、交叉取消恢复、会话隔离、重启恢复、三次装卸无事件残留，以及中途重名回滚均通过。测试使用隔离目录、真实 PTY 与本地模拟服务，未请求真实模型。下一节点为扩展自行创建资源的清理登记，尚未实现。
...
- 外部扩展通过启动环境变量 `LCN_AGENT_EXTENSION` 指定；当前只加载一个入口；可指定 `./dist/extensions/workflow.js` 组合提问与待办，两者共同装卸，不能分别禁用。Node 缓存模块，重新装载不等于刷新源码。
...
2026-09-26 组合扩展集成后运行 `npm run check`、`npm run build`、完整 `npm test` 和 `git diff --check`，均通过。测试使用隔离目录和本地模拟服务，未请求真实模型。阶段 0～4 的历史快照本次只做源码核对与类型检查，未重跑已移除的历史专项验收。
...
| 2026-09-25～26 阶段 5 扩展基础 | 静态注册、外部加载、命令、事件顺序、失败回滚、卸载及重复装载通过；组合入口三次装卸无事件残留，中途重名回滚且保留原注册项 |
